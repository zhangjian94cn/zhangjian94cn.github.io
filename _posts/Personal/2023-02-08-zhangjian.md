...
各位面试老师好，我叫章坚，很高兴有这次的面试机会。接下来，我先做一个简单的自我介绍。

我本科毕业于东南大学，后保研至南京大学。研究生期间，我的主要研究方向是：自动驾驶场景下的高精度语义地图重建，技术主要涉及三个方面：1）机器人的定位与建图（也就是常说的SLAM技术），2）（对信息进行融合的）深度学习技术，3）以及重建可视化过程中的图形学相关技术。在此期间，我分别完成了一篇专利与SCI期刊。此外，研究生期间，我曾在网易游戏渲染引擎组实习，获得了区别于普通offer的special offer。

毕业后，我选择了阿里巴巴的淘系技术部，作为阿里核心的部门之一，从事深度学习与图形学的交叉领域，即可微渲染方面的研究工作。在此期间，我提出了一套基于神经网络的自动化商品3D模型重建方案（Object Drawer），完成了从算法设计，开发，测试，部署以及后续迭代优化的一整套工作。期间，算法成功上线手淘，方案获得了2022年淘宝双十一9大技术亮点之一等荣誉。此外，基于产品应用中的实际问题，我进行算法创新，形成了两篇论文，于22年分别被国际顶级会议CVPR，ECCV接受，我均为第一作者，同年5月，我晋升至高级算法工程师。这也代表了我们的技术受到了国际级认可，处在业内领先水平。

去年7月，我离开阿里来到英特尔，就职于大数据组下的高性能数据分析团队，从事梯度提升决策树的算法加速工作。期间，我深入算法原理与底层实现，提出了一套基于AVX向量化指令的算法加速方案。核心思想是：通过分组方式提升决策树的局部性，使用寄存器实现相对于L1更高一级的缓存，减少内存的存取操作，最终实现了XGBoost 3倍以上的加速。期间，完成一篇论文并获得2022第四季度团队最佳绩效。

以上，是我学习以及工作经历的介绍。简单总结下，通过这几年的工作经历，我觉得最有价值的是：我形成了一套自己的算法设计与开发方法论。这让我在面对实际算法问题时，不仅能够快速的找到解决方案，同时尽可能的从问题本质出发设计算法，避免拆了东墙补西墙，从根本上保证算法在实际应用中的效果。同时，我也一直保持着对技术的热情，持续阅读最新技术的相关论文，从中学习算法设计的核心思想。比如最近很火的chatgpt，我也在自己的服务器上部署了一个小型模型，包括使用flask将模型预测封装成web服务，使用docker实现服务容器化，再使用nginx实现后端服务的反向代理，最后简单做了一个html客户端实现交互。
